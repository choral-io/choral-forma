...
1. Read `knowledge/.workflow/manifest.yml` when present; use its `knowledge_dir`, `agent_skills_dir`, `agent_local_dir`, and `canonical_language`.
2. Read the knowledge workflow block in root `AGENTS.md`.
3. Read `knowledge/README.md`, `knowledge/schemas/common.md`, and the relevant schemas under `knowledge/schemas/`.
4. Read only the knowledge areas needed for the requested mode.
5. Read `knowledge/planning/KANBAN.md` and task items only for delivery-related modes.
...
Exclude `knowledge/workspace/*/local/**` by default. Read local workspace files only when the user explicitly asks about their own local state or the mode requires current-member local context.
