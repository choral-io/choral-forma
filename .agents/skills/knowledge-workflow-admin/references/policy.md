...
Use this reference for the maintainer policy mode.

Policy mode manages project-specific Agent behavior rules inside root `AGENTS.md` -> `Project-Specific Knowledge Workflow`. Use `knowledge-assistant` for ordinary read-only policy explanation and audit. Use this maintainer workflow only when the user asks to define, update, or save policy, or accepts a recommendation after a workflow finds missing project policy.
...
If the user asks general workflow education or read-only policy explanation, use `knowledge-assistant` instead.
...
<optional: ask a maintainer to define or update stable policy>
```
