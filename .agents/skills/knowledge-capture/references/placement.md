...
- Store design images and exports in `<knowledge_dir>/assets/design/<feature-name>/` and link to them from Markdown files.
...
- Create member profiles in `<knowledge_dir>/members/` when the user approves a project-visible member record. Use `<knowledge_dir>/members/templates/member.md.tpl`; ask for or confirm `member_id`, `display_name`, public responsibilities, focus areas, and collaboration context.
- Create group documents in `<knowledge_dir>/groups/` when the user approves a project-visible team, review board, maintainer group, or working group. Use `<knowledge_dir>/groups/templates/group.md.tpl`; ask for or confirm `group_id`, `display_name`, purpose, responsibility scope, owners, and members.
