...
```text
knowledge/workspace/<member-id>/local/logs/YYYY-MM-DD.md
```
...
- Use `rg <work-id> knowledge/workspace/<member-id>/local/logs/` to gather history.
...
- Use `knowledge/workspace/<member-id>/summaries/` for ordinary summaries.
- Use `knowledge/workspace/<member-id>/handoffs/` when another member needs to continue the work.
