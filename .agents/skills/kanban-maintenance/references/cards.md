...
Resolve `[[example-delivery-task]]` to `<knowledge_dir>/tasks/items/example-delivery-task.md` by default. If multiple canonical files match the same id, report ambiguity instead of guessing.
...
| -------------------------------- | ------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------- |
| `Backlog -> Ready`               | linked task has `readiness: ready`; no unresolved `blocked_by`; Ready Checklist satisfied                     | none required; propose metadata fixes first if not ready                      |
| `Backlog -> Doing`               | maintainer explicitly approved immediate start; assignee/current handler confirmed; Ready Checklist satisfied | confirm or set `assignees` through approved task metadata change              |
