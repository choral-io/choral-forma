...
Use member or group wikilinks in responsibility metadata, such as `[[members/Gavroche]]` or `[[groups/{{default_group_id}}]]`. Manual short wikilinks such as `[[Gavroche]]` are valid only when they resolve uniquely in the expected member or group scope. Use the manifest `default_group_id` group when ownership belongs to the default group and no more specific member or group owner is assigned.
...
Canonical-language files are the source of truth. This repository records `canonical_language: {{canonical_language}}` in `{{knowledge_dir}}/.workflow/manifest.yml`. Localized files must use BCP 47 suffix casing, such as `.zh-CN.md`.
...
- Use path-qualified wikilinks for tool-written knowledge references, such as `owners: ["[[groups/{{default_group_id}}]]"]` or `related_to: ["[[tasks/items/example-note]]"]`.
