...
Spaces are not a hardcoded product partition. This getting-started workspace configures `spaces` as a primary taxonomy in `.forma/spaces/index.md`, then defines each term in `.forma/spaces/*.md`:
...
In this getting-started workspace, the spaces also reinforce a practical workflow: tasks can refer to [[members/mira-chen|members]], and shared guidelines such as [[guidelines/task-selection|Task Selection]] stay close to the work they shape.
