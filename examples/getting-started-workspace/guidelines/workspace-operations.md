...
summary: "How to copy, review, and maintain the getting-started workspace as a shared team space."
owners:
  - members/mira-chen
reviewers:
  - members/sam-rivera
...
Use this guideline when you first copy the getting-started example, when you review the shared structure, or when you decide whether a change belongs in committed workspace content.
...
6. Keep personal configuration in explicitly included private paths when the copied workspace defines them. This getting-started workspace includes optional `.forma/local/*.yml` and `.forma/local/*.md` patterns in `.forma.md`, and its `.gitignore` keeps `.forma/local/` uncommitted. Do not treat ignored files as shared workspace content.
