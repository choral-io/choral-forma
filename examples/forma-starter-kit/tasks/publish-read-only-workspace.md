---
title: "Publish Read-only Workspace"
summary: "Serve the workspace locally and share the read-only browser with the team."
status: "done"
readiness: "done"
priority: "low"
owners:
  - members/sam-rivera.md
assignees:
  - members/sam-rivera.md
reviewers:
  - members/mira-chen.md
blockedBy: []
createdAt: "2026-06-03T18:00:00Z"
updatedAt: "2026-06-03T18:00:00Z"
dueDate: "2026-06-01"
---

# Publish Read-only Workspace

This completed item keeps the kanban view realistic by showing a Done column.

The basic workflow is:

1. run or restart `forma serve`;
2. let the server build or refresh the read model from repository files;
3. open the local WebApp;
4. share the repository and instructions with the team.

This task now serves as a reference example for [[guidelines/workspace-operations|Workspace Operations]] and the walkthrough in [[notes/getting-started|Getting Started]].
