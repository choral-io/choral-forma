...
This workspace is a small, copyable starter for Choral Forma. It is designed to show how a team can organize notes, tasks, members, and guidelines with ordinary Markdown plus a single `.forma.md` entry point.
...
- `.forma/local/*.yml` and `.forma/local/*.md`: optional private configuration loaded only because this starter's `.forma.md` explicitly includes those patterns.
...
Workspace-level guidelines live in `.forma.md`. In this starter, workspace operations and task selection are loaded together because Agents often need those procedures before they inspect a specific page. Individual spaces can still add more specific guidance when a rule truly applies only inside that space.
