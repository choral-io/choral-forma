...
test("the exact window boundary is accepted and a late event is rejected", () => {
  ...
  assert.equal(classifyAcknowledgement({ id: "boundary", delaySeconds: 120, replayed: false }, config).decision, "accepted");
  assert.equal(classifyAcknowledgement({ id: "late", delaySeconds: 121, replayed: false }, config).decision, "rejected");
});
