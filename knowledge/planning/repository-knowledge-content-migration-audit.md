...
The original migration design listed the smaller target baseline: `product`, `architecture`, `decisions`, `concepts`, `discovery`, `guidelines`, `planning`, `tasks`, `members`, and `workspace`.
...
- `knowledge/workspace/*/index.md` is the configured workspace include.
- `knowledge/workspace/*/handoffs/**/*.md` and `knowledge/workspace/*/research/**/*.md` are working context paths and are not indexed by default.
- `knowledge/workspace/*/local/**` remains local-only by repository ignore policy and is not a Forma runtime convention.
- Workspace research can remain working evidence. Durable synthesis should still be promoted to a canonical space such as `discovery`, `architecture`, or `product` when it becomes current product direction.
...
Configured workspace pages:

- `knowledge/workspace/tiscs/handoffs/forma-markdown-parser-spike.md`
- `knowledge/workspace/tiscs/handoffs/forma-starter-kit-to-system-refactor.md`

Promotion candidates:

- The Markdown parser spike handoff and research report are historical technical evidence. If still useful, the durable synthesis should move to `discovery` or `architecture`; otherwise it can remain workspace or be left as historical context.
...
3. Classify workspace material once current language and relationship metadata are stable enough to judge what should be promoted.
