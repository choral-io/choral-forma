...
| `knowledge/workspace/*/research/**` | Defer | Research notes are not currently configured as a shared space. Promote durable synthesis to `discovery` or `architecture` only when a current decision or implementation task needs it. |

## Outcome

No shared content move is needed for the current migration slice.

The configured handoffs remain shared support material. Local-only files remain outside shared migration, and research material remains deferred until a concrete product or architecture task needs its evidence.
