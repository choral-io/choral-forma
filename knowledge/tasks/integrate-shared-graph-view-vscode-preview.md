...
- Verified the shared Graph runtime (33 focused tests), the complete 200-test workspace Vitest suite and 23 Node tooling tests, production browser bundle, packaged VSIX installation, native Preview reload, light/dark theme switching with an active selection, persistent static focus edges, and native source navigation. The packaged development VSIX is 199.29 KB; `markdown-preview.js` is 203.27 KB.
...
Residual validation remains for a real Remote Extension Host, live high-contrast and reduced-motion sessions, long-running memory and idle-CPU profiling, and the planned 25/500/5,000-node cross-host performance gates. These remain explicit internal-Alpha boundaries under [[tasks/validate-shared-graph-view-cross-host-parity]]. The project Graph's default relationship filters also need a separate product decision; assignment metadata has not been silently excluded from the configured View.
