...
This document defines how project knowledge becomes delivery work tracked in `knowledge/planning/KANBAN.md`.
...
Task items live in `knowledge/tasks/items/` and follow `knowledge/schemas/tasks.md`.
...
| --------------------- | ------------------------------------------------ | ------ | -------- | -------- | ------------------------- | -------- | ------ |
| Example delivery task | `knowledge/tasks/items/example-delivery-task.md` | app    | P1       | Sprint 1 | `[[groups/default-team]]` | None     | Ready  |

Use `.agents/skills/next-task-selection/SKILL.md` to recommend accepted Kanban cards. Selection is read-only by default:
...
Use `.agents/skills/workspace-worklist/SKILL.md` when a member takes accepted work into local execution.
...
Use `.agents/skills/delivery-review/SKILL.md` before Done for:
...
- Never edit `knowledge/planning/KANBAN.md` without explicit maintainer approval.
