...
Decide and implement the shared search capability behind Quick Open when it needs to become more than dashboard-local route navigation.
...
- Add a lightweight read-only entry search operation, such as `search.entries`.
    - Search candidates come from the runtime discovery/in-memory projection, not raw Markdown body text.
    - Match against path, title, summary, space, and kind.
    - Return workspace-relative POSIX paths, display titles, and simple match fields when cheaply available.
...
- Preserve the current dashboard-local Quick Open behavior if shared search is deferred.
- Add focused tests for matching, empty queries, no-result behavior, and invalid parameters.
- Update architecture or product knowledge if the operation name and result shape become contract commitments.
...
- Users can search discovered entries by title or path and open a selected result.
- Empty queries, no-result searches, discovery diagnostics, and invalid parameters produce clear empty states or diagnostics rather than panics.
- Search results use workspace-relative POSIX paths only.
- Search ranking is deterministic for unchanged workspace input.
...
- Should the first quick switcher search be exposed through CLI as well as RPC, or remain WebApp/RPC-only until there is script demand?
- Should the first public release keep dashboard-local Quick Open and defer `search.entries`?
