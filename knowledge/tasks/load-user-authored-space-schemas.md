...
| Checks run | `cargo test`, Prettier check for changed knowledge files, `forma check --json`, `forma config inspect --json`, `forma tasks list --json`, and `forma workspace health --json` passed or returned the existing accepted health warning baseline. |
