...
- Before writing knowledge, read `knowledge/.workflow/schemas/common.md` and the target area schema.
- Before changing delivery status, read `knowledge/.workflow/rules/delivery.md`.
- Before using member-local material, read `knowledge/.workflow/rules/workspace.md`.
...
| Workflow rules        | `knowledge/.workflow/rules/*.md`      | Operational rules, not project facts.            |
| Schemas               | `knowledge/.workflow/schemas/*.md`    | Writing contracts, not project facts.            |
| Templates             | `knowledge/.workflow/templates/*.md`  | Starting points, not project facts.              |
...
When sources conflict, apply precedence from `knowledge/.workflow/schemas/common.md`. Local notes and current conversation can trigger updates, but they do not replace canonical knowledge until approved capture or maintenance happens.
...
Store binary or exported supporting materials under `knowledge/assets/<asset-type>/<topic>/`, for example `knowledge/assets/design/<feature-name>/`, and link to them from Markdown files.
