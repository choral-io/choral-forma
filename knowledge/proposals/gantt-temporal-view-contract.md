...
Connectors leaving one predecessor are separated by lanes. Without lanes, edges from one entry whose target rows lie on the same side share an initial riser; that trunk is expected fan-out geometry, not a collision. The earlier aggregate coverage metric mixed such trunks with independent overlaps. Its historical analysis did not preserve a repeatable classifier or denominator, so it is retired rather than used as a defect rate. Lanes are an arity cue intended to make outgoing relationships easier to distinguish at their source, not a fix for a measured collision. Six lanes three pixels apart comes from two geometric boundaries rather than from that metric: eight lanes reach past a successor's start, and a step that divides a day width makes connectors from different predecessors realign on one offset. Route selection is still decided on the unshifted dates. For a direct route, clamp each lane to the corridor that remains after reserving an eight-pixel approach to the successor; when a short gap has room only for the base riser, its edges share that lane. A detour around overlapping intervals always uses the base riser, so a lane cannot lengthen its return segment or push it farther through a bar.
