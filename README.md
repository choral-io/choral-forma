...
```sh
curl -fsSL https://raw.githubusercontent.com/choral-io/choral-forma/main/install.sh | sh -s -- v0.1.0-alpha.20
```
...
& ([scriptblock]::Create($script.Content)) -Version v0.1.0-alpha.20
```
...
mise use github:choral-io/choral-forma@0.1.0-alpha.20
mise install github:choral-io/choral-forma@0.1.0-alpha.20
forma --version
```
...
```toml
[tools]
"github:choral-io/choral-forma" = "0.1.0-alpha.20"
```

Mise normally autodetects the matching GitHub Release asset from OS and architecture. During the alpha stage, pin a release version because `latest` does not resolve prerelease-only repositories. GitHub release tags use the `v0.1.0-alpha.20` form, while mise normalizes the GitHub backend tool version to `0.1.0-alpha.20`. If autodetection is not enough for a team's environment, add platform-specific `asset_pattern` values as described in the [mise GitHub backend documentation](https://mise.jdx.dev/dev-tools/backends/github.html).
