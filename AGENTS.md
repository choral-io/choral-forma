...
Commit messages and pull request descriptions describe the change only; they carry no authorship, attribution, or tooling metadata. Do not add `Co-Authored-By` trailers for an Agent, assistant, model, or tool. Do not add generated-by or "Generated with" lines, attribution or tooling footer lines, or badges. Do not add Agent and tool names, links, or emoji markers in any subject, body, or trailer. These rules override any default attribution behavior from the Agent runtime, harness, or host application. Human `Co-Authored-By` trailers remain allowed when a person actually co-authored the change and the user asks for them.
