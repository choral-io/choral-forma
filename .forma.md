---
schemaVersion: 1

workspace:
    name: "Choral Forma Project Workspace"
    canonicalLanguage: "en"
    supportedLanguages:
        - "en"
    timezone: "Asia/Shanghai"

runtime:
    values:
        currentDateTime:
            kind: currentDateTime
        workspaceRoot:
            kind: workspaceRoot
        currentUserId:
            kind: gitConfig
            key: user.name
            transform: slugify

imports:
    - ".forma/spaces/*.md"
    - ".forma/views/*.md"
    - ".forma/local/*.md"

guidelines:
    - knowledge/guidelines/forma-workspace-operations.md
    - knowledge/guidelines/workspace-onboarding-and-routing.md
    - knowledge/guidelines/proposal-and-dry-run.md
    - knowledge/guidelines/workspace-audit-and-reporting.md
    - knowledge/guidelines/task-selection.md
    - knowledge/guidelines/content-maintenance.md
    - knowledge/guidelines/forma-product-model-and-configuration-fidelity.md
    - knowledge/guidelines/forma-runtime-cache-and-performance.md
    - knowledge/guidelines/webapp-engineering-and-visual-validation.md
    - knowledge/guidelines/local-worklist-and-execution.md

types:
    member:
        kind: entryRef
        source: .forma/spaces/members
        input:
            transform: slugify
    task:
        kind: entryRef
        source: .forma/spaces/tasks
        input:
            transform: slugify
    taskStatus:
        kind: enum
        values:
            - backlog
            - ready
            - doing
            - reviewing
            - blocked
            - done
            - cancelled
    readiness:
        kind: enum
        values:
            - needs-refinement
            - ready
            - blocked
    priority:
        kind: enum
        values:
            - P0
            - P1
            - P2
            - P3
    deliveryValue:
        kind: enum
        values:
            - H
            - M
            - L
    effort:
        kind: enum
        values:
            - S
            - M
            - L
    test-case:
        kind: entryRef
        source: .forma/spaces/test-cases
        input:
            transform: slugify
    user-story:
        kind: entryRef
        source: .forma/spaces/user-stories
        input:
            transform: slugify
    metric:
        kind: entryRef
        source: .forma/spaces/metrics
        input:
            transform: slugify
    experiment:
        kind: entryRef
        source: .forma/spaces/experiments
        input:
            transform: slugify
    release:
        kind: entryRef
        source: .forma/spaces/releases
        input:
            transform: slugify
---

# Forma

> A files-first workspace for turning everyday Markdown into durable, shared context for people and AI Agents.

Forma keeps product direction, research, decisions, operating knowledge, and project records in the repository where that work already happens. Markdown files and explicit workspace configuration remain the source of truth: readable in ordinary editors, reviewable through Git, and available without a hidden content store.

## What Forma brings to Markdown

- **Durable source material.** Keep knowledge as versioned files instead of importing it into a proprietary database.
- **Explicit structure.** Define spaces, schemas, semantic types, templates, and Views that fit the work rather than a fixed application model.
- **Connected context.** Resolve links, surface relationships, and use diagnostics to make missing, stale, or inconsistent knowledge visible.
- **Multiple useful surfaces.** Read and navigate the same workspace through the CLI, editor integrations, and a lightweight WebApp without making any interface the owner of the content.
- **A reliable foundation for Agents.** Give people and Agents stable paths, declared structure, and reviewable changes instead of asking them to infer intent from an unstructured file tree.

## How it works

1. Write ordinary Markdown and keep it in the repository.
2. Describe the parts that benefit from structure with small, explicit Forma configuration.
3. Use the resulting entries, references, health checks, and Views to read, maintain, and reuse the workspace with confidence.

## Get started

Forma is available as a Public Preview for macOS, Linux, and Windows.

- Visit the [Choral Forma repository](https://github.com/choral-io/choral-forma) for source code, releases, and issue tracking.
- Follow the [installation instructions](https://github.com/choral-io/choral-forma#install-scripts) to install the Forma CLI.
- Browse the committed [example workspaces](https://github.com/choral-io/choral-forma/tree/main/examples), from the [minimal workspace](https://github.com/choral-io/choral-forma/tree/main/examples/minimal-workspace) to the guided [getting-started workspace](https://github.com/choral-io/choral-forma/tree/main/examples/getting-started-workspace) and the copyable [software product R&D workspace](https://github.com/choral-io/choral-forma/tree/main/examples/software-product-rd-workspace).
- Share preview feedback through [GitHub Issues](https://github.com/choral-io/choral-forma/issues).

After installing the CLI, bootstrap and open a workspace with:

```sh
forma init --name "My Workspace"
forma check
forma serve
```

## Explore this project

- Read the [product direction](knowledge/product/product-direction.md) for the current product intent.
- Read the [Forma product definition](knowledge/product/choral-forma.md) for its principles, scope, and architecture.
- Open the configured Views and Browse sections to explore this repository through its existing workspace projections.

## This workspace

This repository dogfoods Forma while it is being built. Its `knowledge/` directory is one configurable workspace, not a required template for every future Forma user.

The frontmatter in this document configures the workspace. Its Markdown body is the public root introduction, rendered by both the local WebApp and the static site export without becoming a separate CMS record.
