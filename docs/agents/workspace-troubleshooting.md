...
4. Treat directory names and ignored files as organization conventions, not privacy or publication guarantees. A configured file participates according to its configuration.
5. For migration or configuration diagnostics, load `forma docs get workspace.configuration`. Propose the smallest correction and wait for approval before changing shared workspace material.
